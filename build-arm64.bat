@echo off
echo ========================================
echo Building Finger Screensaver with Aggressive Trimming for ARM64
echo ========================================
echo.
...
dotnet publish -c Release -r win-arm64 --self-contained -p:PublishTrimmed=true -p:TrimMode=full -p:PublishSingleFile=true -p:StripSymbols=true -p:EnableCompressionInSingleFile=true -p:DebugType=None -p:DebugSymbols=false -p:PublishReadyToRun=false -p:IncludeNativeLibrariesForSelfExtract=true -o bin\Release\win-arm64
if %errorlevel% neq 0 (
...
echo ========================================
echo.
echo Screensaver file: bin\Release\FingerScreensaver-arm64.scr
echo.
