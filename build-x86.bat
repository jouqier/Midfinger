@echo off
echo ========================================
echo Building Finger Screensaver with Aggressive Trimming for x86
echo ========================================
echo.
...
dotnet publish -c Release -r win-x86 --self-contained -p:PublishTrimmed=true -p:TrimMode=full -p:PublishSingleFile=true -p:StripSymbols=true -p:EnableCompressionInSingleFile=true -p:DebugType=None -p:DebugSymbols=false -p:PublishReadyToRun=false -p:IncludeNativeLibrariesForSelfExtract=true -o bin\Release\win-x86
if %errorlevel% neq 0 (
...
echo ========================================
echo.
echo Screensaver file: bin\Release\FingerScreensaver-x86.scr
echo.
